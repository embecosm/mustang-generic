...
with lib;
with builtins; let
  baseVersion = lib.fileContents "${src}/gcc/BASE-VER";
  majorVersion = lib.versions.major baseVersion;
  gitSuffix =
    if gitRev != null
    then "-g${gitRev}"
    else
  ...
  # This cannot have a space in it since it's passed as a configure flag.
  pkgVersion = "Rivos${gitSuffix}";
  ...
  stdenv.mkDerivation (
    {
      ...
      # flex is required when building from git.
      nativeBuildInputs =
        [flex texinfo which gettext python3]
      ...
      preConfigure =
        (import ../common/pre-configure.nix {
          inherit lib;
